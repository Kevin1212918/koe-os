...
impl MemoryManager for X86_64MemoryManager {
    fn init() -> Self {
        ...
        unsafe {
            pml4 = TableRef::from_raw(
                Level::PML4,
                PML4_TABLE.get().as_mut_unchecked(),
            );
            pdpt = TableRef::from_raw(
                Level::PDPT,
                PDPT_TABLE.get().as_mut_unchecked(),
            );
            pd = TableRef::from_raw(
                Level::PD,
                PD_TABLE.get().as_mut_unchecked(),
            );
        }
        ...
        while kernel_map_addr < kernel_end_vma() {
            ...
            unsafe {
                kernel_pd_ent
                    .reinit(kernel_page_paddr, KERNEL_PAGE_FLAGS)
                    .unwrap_or_else(|| {
                        panic!(
                            "kpd fail: {:#x}",
                            kernel_page_paddr.usize()
                        )
                    });
            }
        ...
        let mut mapping_ent = pml4.index_with_vaddr(Addr::<RecursivePagingSpace>::new(
            RecursivePagingSpace::RANGE.start,
        ));
...
impl<'a, T: VirtSpace> Walker<'a, T> {
    ...
    unsafe fn new(page_structure: &'a mut PageStructure, target_vaddr: Addr<T>) -> Self {
...
impl PageStructure {
    ...
    fn get_cr3_ent(&mut self) -> EntryRef<'_> {
        unsafe { EntryRef::from_raw(&mut self.0, Level::CR3) }
    }
