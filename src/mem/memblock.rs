...
impl PartialEq for Memblock {
    fn eq(&self, other: &Self) -> bool {
        self.is_present == other.is_present && (self.is_present == 0 || self.base == other.base)
    }
}
...
impl Ord for Memblock {
    fn cmp(&self, other: &Self) -> core::cmp::Ordering {
        self.partial_cmp(other)
            .expect("Memblock order should be total")
    }
}
impl Memblock {
...
impl Memblocks {
    ...
    fn put_block(&mut self, block: Memblock) -> bool {
        ...
        if pivot > 0 {
            ...
            if block.base - prev.base < prev.size {
                return false;
            }
            if block.base - prev.base == prev.size {
        ...
        if !next.is_empty() {
            if next.base - block.base < block.size {
                return false;
            }
            if next.base - block.base == block.size {
    ...
    fn cut_block(&mut self, layout: Layout, at: Option<usize>) -> Option<Memblock> {
        fn find_cut_block_in_range(data: &[Memblock], layout: Layout) -> Option<(usize, usize)> {
        ...
        let (cut_idx, cut_base) = match at {
            ...
            Some(at) => {
                ...
                find_cut_block_in_range(&self.data[at_idx..self.len], layout)
                    .or_else(|| find_cut_block_in_range(&self.data[0..at_idx], layout))?
            },
...
impl<'boot> BootMemoryManagerBuilder {
    pub fn new() -> Option<BootMemoryManagerBuilder> {
        ...
        unsafe {
            free_blocks = Memblocks::new(FREE_BLOCKS.get().as_mut_unchecked().as_mut_slice());
            reserved_blocks =
                Memblocks::new(RESERVED_BLOCKS.get().as_mut_unchecked().as_mut_slice());
        }

        let mba = BootMemoryManager(spin::Mutex::new(Inner {
            free_blocks,
            reserved_blocks,
        }));
    ...
    pub fn add_free(&self, base: usize, size: usize) -> bool {
        self.0
            ...
            .lock()
            .free_blocks
            .put_block(Memblock::new(base, size))
    }
    ...
    pub fn build(self) -> BootMemoryManager { self.0 }
}
