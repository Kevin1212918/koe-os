...
use entry::{EntryRef, EntryTarget, Flags, RawEntry};
...
pub static MMU: spin::Mutex<Option<MemoryManager>> = spin::Mutex::new(None);
...
static DEFAULT_KERNEL_MAP: spin::Once<MemoryMap> = spin::Once::new();
...
pub fn init(bmm: &BootMemoryManager) {
    ...
    DEFAULT_KERNEL_MAP.call_once(|| MemoryMap {
...
impl paging::MemoryManager for MemoryManager {
    ...
    fn map(&mut self) -> &MemoryMap { &self.0 }
...
impl paging::MemoryMap for MemoryMap {
    fn new() -> Self {
    ...
    unsafe fn map<V: VirtSpace>(
        ...
        self.using_entry_ref(|cr3| {
            ...
            walker.cur().reinit(ppage.start(), Flags::from(attr));
        });
